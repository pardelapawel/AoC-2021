...
def is_border(cave):
    return cave in 'start end'.split()
    ...
    if node_from == 'end':
        ...
        for node in graph[node_from]:
            if node not in visited:
                ...
                continue
            smalls = [x for x in path if is_small(x)]
            doubles = len(smalls) - len(set(smalls))
            #if is_border(node):
            #    paths = find_all_paths__(graph, node, visited.copy(), path.copy())
            if smalls.count(node) == 1 and doubles == 0:
                paths = find_all_paths__(graph, node, visited.copy(), path.copy())
            else:
                continue
            if node == 'start':
                continue
            print(path, node)
            print(f'{smalls.count(node)} == 1 and {doubles} == 1')
            all_paths.extend(paths)
    return all_paths
